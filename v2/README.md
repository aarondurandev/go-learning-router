...
v2 replaces the linear route slice with a radix tree for O(log n) matching.

## Installation

```bash
go get github.com/aarondurandev/go-learning-router/v2
```

## Design notes

- Routes are stored in a radix tree and matched in O(log n) time — each request walks one node per path segment regardless of how many routes are registered.
